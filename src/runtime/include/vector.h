...
#define wrench_vec_type(type) \
    struct { type *data; uint32_t length; uint32_t capacity; }

#define wrench_vec_init(vec) \
    ((vec)->data = NULL, (vec)->length = 0, (vec)->capacity = 0)

#define wrench_vec_deinit(vec) \
    wrench_deallocate((vec)->data)

#define wrench_vec_unpack(vec) \
    (char **)&(vec)->data, &(vec)->length, &(vec)->capacity, sizeof(*(vec)->data)

#define wrench_vec_push(vec, val) \
    (wrench_vec_assure_size(wrench_vec_unpack(vec)) == 0 ?   \
     ((vec)->data[(vec)->length++] = (val)), 0 : \
    -1)

#define wrench_vec_push_array(vec, arr, count)                      \
    do {                                                        \
        (vec)->length += (uint32_t) count;                                 \
        if (0 != wrench_vec_assure_size(wrench_vec_unpack(vec))) break; \
        uint32_t n__ = (count);                                 \
...
#define wrench_vec_pop(vec) \
    ((vec)->data[0 < (vec)->length ? --(vec)->length : 0])

#define wrench_vec_last(vec) \
    ((vec)->data[(vec)->length - 1])

#define wrench_vec_reset(vec) \
    ((vec)->length = 0)

#define wrench_vec_foreach(vec, var, iter)                                     \
    if  ((vec)->length > 0 )                                            \
    for ( (iter) = 0;                                                   \
